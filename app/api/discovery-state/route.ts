import { createPublicDiscoveryCatalog, parseDiscoveryInteractionState } from "@/lib/public-discovery";
...
export async function GET() {
  try {
    ...
    return Response.json(await readDiscoveryState(supabase, userId, catalog), { headers: { "Cache-Control": "private, no-store" } });
  } catch (cause) {
...
export async function PUT(request: Request) {
  try {
    ...
    const state = parseDiscoveryInteractionState(JSON.stringify(body.state), catalog);
    const { supabase, userId } = await requireAuthenticatedUser();
    const result = await replaceDiscoveryState(supabase, userId, state, body.expectedRevision as number, catalog);
