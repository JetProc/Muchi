...
async function searchTrack(track: ExportTrack, token: string): Promise<Candidate[]> {
  if (track.provider === "youtube" && typeof track.providerTrackId === "string") return [{ id: track.providerTrackId, title: track.title, artist: track.artist, album: track.album }];
  const response = await youtube(token, `/search?part=snippet&type=video&maxResults=5&q=${encodeURIComponent(`${track.title} ${track.artist}`)}`);
  if (!response.ok) return [];
  const body = await response.json() as { items?: Array<{ id?: { videoId?: string }; snippet?: { title?: string; channelTitle?: string } }> };
  return (body.items ?? []).flatMap((item) => item.id?.videoId ? [{ id: item.id.videoId, title: item.snippet?.title ?? track.title, artist: item.snippet?.channelTitle ?? track.artist, album: "" }] : []);
}
...
export async function POST(request: Request) {
  try {
    ...
    if (body.action === "match") return Response.json({ matches: await Promise.all(body.tracks.map(async (track) => ({ trackId: track.id, candidates: await searchTrack(track, body.accessToken as string) }))) });
