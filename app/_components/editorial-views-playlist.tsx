...
type TrackMatch = { trackId: TrackId; candidates: Candidate[]; selectedId: string | null; errorCode?: string };
...
export function PlaylistBuilder({ archive, chapterId, playlistSource, initialServiceId, step, onStepChange }: {
  ...
  async function connectAndMatch(forceReconnect = false) {
    ...
    try {
      ...
      const response = await fetch(`/api/playlist/${serviceId}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "match", accessToken: token, tracks: selectedEntries.map((entry) => entry.track) }) });
      const body = await responseJson(response) as { matches?: Array<{ trackId?: string; candidates?: Candidate[]; errorCode?: string }>; errorCode?: string };
      setYoutubeErrorCode(body.errorCode);
      setError(body.errorCode ? youtubeErrorMessage(body.errorCode) : null);
      setMatches((body.matches ?? []).map((match) => ({ trackId: match.trackId as TrackId, candidates: match.candidates ?? [], selectedId: match.candidates?.[0]?.id ?? null, errorCode: match.errorCode })));
  ...
  return <div ref={viewRef} className="page-content playlist-builder-view"><header className="playlist-builder-header"><div><span className="section-label">플레이리스트로 내보내기</span><h1>{STEP_LABEL[step - 1]}</h1></div><span className="playlist-step-count" aria-label={`${step}단계, 전체 3단계`}>{step} / 3</span></header><ol className="playlist-stepper" aria-label="플레이리스트 생성 단계">{STEP_LABEL.map((label, index) => <li className={step >= index + 1 ? "is-active" : ""} key={label}><span>{index + 1}</span><em>{label}</em></li>)}</ol>
    ...
    {error ? <p className="auth-gate-error" role="alert">{error}</p> : null}<div className="playlist-builder-actions">{step === 1 ? <button className="button button-primary" type="button" disabled={!selectedTrackIds.length || !playlistName.trim()} onClick={() => onStepChange(2)}>다음</button> : null}{step === 2 ? <button className="button button-primary" type="button" disabled={selectedService.status === "soon"} onClick={() => onStepChange(3)}>{selectedService.name}으로 계속</button> : null}{step === 3 && !matches.length ? <button className="button button-primary" type="button" disabled={loading} onClick={() => void connectAndMatch()}>{loading ? "서비스 연결 중…" : `${selectedService.name} 연결하고 곡 찾기`}</button> : null}{step === 3 && matches.length && youtubeErrorCode ? <button className="button button-primary" type="button" disabled={loading} onClick={() => void connectAndMatch(canReconnectYoutube(youtubeErrorCode))}>{loading ? canReconnectYoutube(youtubeErrorCode) ? "권한 연결 중…" : "다시 검색 중…" : canReconnectYoutube(youtubeErrorCode) ? "Google YouTube 권한 다시 연결" : "다시 검색"}</button> : null}{step === 3 && matches.length && !youtubeErrorCode ? <button className="button button-primary" type="button" disabled={!matchedCount || loading} onClick={() => void exportPlaylist()}><ListMusic size={16} aria-hidden="true" />{loading ? "만드는 중…" : `${matchedCount}곡 내보내기`}</button> : null}</div></div>;
}
